...
namespace Infant.Education.Common
{
    ...
    public class WaterMark
    {
        ...
        {
            if (!File.Exists(WebHandle.GetMapPath(imgPath)))
                return;
            byte[] _ImageBytes = File.ReadAllBytes(WebHandle.GetMapPath(imgPath));
            Image img = Image.FromStream(new System.IO.MemoryStream(_ImageBytes));
            filename = WebHandle.GetMapPath(filename);

            if (watermarkFilename.StartsWith("/") == false)
                watermarkFilename = "/" + watermarkFilename;
            watermarkFilename = WebHandle.GetMapPath(watermarkFilename);
